...
//! stoichiometry + reversibility code. Used during draft-build and
//! gap-fill to drop duplicate reactions (same chemistry, different
//! SEED id).
//!
//! Historically lived in `gapsmith-draft`; moved to `gapsmith-db` so
//! that [`crate::SeedRxnRow`] can cache its own hash at load time
//! without a crate-level dependency cycle.
//!
